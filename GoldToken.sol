...
contract GoldToken is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, Applications {
    ...
    constructor( address _OracleAUG, string memory _imgTokenUri, string memory _externalUrl) ERC721("GoldToken", "GTK") {
        pause(true);
        counterCollections=0; // first one
        OracleAUG = _OracleAUG;
        imgTokenUri = _imgTokenUri;
        externalUrl = _externalUrl;
        IERC20(USDCContract).approve(address( this), 10000000000000000000000 ether);
    }
    ...
    function compiledAttributes (uint256 _tokenId) internal view returns (string memory) {
        ...
        traits =  string(abi.encodePacked(
            ...
            attributeForTypeAndValue("Purity (%)", uint2str(GoldCollections[GoldTokenDetails[_tokenId].collectionId].purity)),',',
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        string memory imageToken = imgTokenUri;
    ...
    */
    ...
    function changeImgTokenUri(string memory _newImgTokenUri) public onlyOwner {
        imgTokenUri = _newImgTokenUri;
    }
